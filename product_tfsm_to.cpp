...
{
    for (auto mutationTransition : this->mutationMachine->transitions) {
        if (mutationTransition.src == state->mutationState) {
            Transition * related = NULL;
            for (auto specificationTransition : this->specification->transitions) {
                if (specificationTransition.src == state->specificationState && specificationTransition.i == mutationTransition.i) {
    ...
    for (auto specificationTimeout : this->specification->timeouts) {
        if (specificationTimeout.src == state->specificationState) {
    ...
    for (auto mutationTimeout : this->mutationMachine->timeouts) {
        if (mutationTimeout.src == state->mutationState) {
...
std::vector<path> Product_TFSM_TO::revealingPaths(sequence alpha)
{
    vector<path> results;
    path currentPath;
    revealingPathsRecursive(this->initialState, currentPath, results, alpha, 0, 0, false);
    return results;
}
...
/*
void Product_TFSM_TO::revealingPathsRecursive(ProductState * state, path currentPath, vector<path> &results, sequence alpha, int sequenceIndex, int timeBuffer, bool canInput)
{
        ...
        if (!canInput) {
            ...
            for (auto transition : this->transitions) {
                if (transition.src == state->getKey() && transition.isTimeout) {
                    ...
                    if (timeout <= t) {
                        canHaveOneTimeout = true;
                        path newPath(currentPath);
                        newPath.push_back(transition.id);
                        if (this->isPathDeterministic(newPath)) {
                            this->revealingPathsRecursive(tgtNode, newPath, results, alpha, sequenceIndex, timeBuffer + timeout, false);
                        }
                    }

                }
            }
            if (!canHaveOneTimeout) {
                for (auto mutaTimeout : this->mutationMachine->timeouts) {
                    if (mutaTimeout.src == state->mutationState) {
                        path newPath(currentPath);
                        newPath.push_back(mutaTimeout.id);
                        if (this->isPathDeterministic(newPath)) {
                            this->revealingPathsRecursive(state, newPath, results, alpha, sequenceIndex, symbol_time, true);
                        }
                    }
                }
            }
                ...
                if (transition.src == state->getKey()) {
                    if (transition.src == state->getKey() && !transition.isTimeout) {
                        if (transition.i == symbol) {
                            ...
                            newPath.push_back(transition.id);
                            if (this->isPathDeterministic(newPath))
                                this->revealingPathsRecursive(tgtNode, newPath, results, alpha, sequenceIndex+1, 0, false);
                        }
                    }
                }
            }
...
deque<ProductTransition> Product_TFSM_TO::Dijkstra()
{
    ...
    distances.find(this->initialState->getKey())->second = 0;
    ...
    while (currentStateKey != initialState->getKey()) {
...
sequence Product_TFSM_TO::inputSequenceFromAcceptedLanguage()
{
    sequence input;
    if (!this->hasNoSinkState) {
        deque<ProductTransition> res = Dijkstra();
        int time = 0;
